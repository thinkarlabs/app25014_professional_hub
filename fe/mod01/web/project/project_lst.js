...
_cp.init = function(){
	...
	_cp.views.page = './fe/app/25014/mod01/web/project/project_lst.htm';
	_cp.api.list = '/be/app/25014/api/app25014_professional_hub/be/mod01/project/';
	...
	_app.bind_event('#status','changed',_cp.on.filter_list);
...
_cp.on.filter_list = function() {
	...
	_app.get(_cp.api.list + _filter, function( data ) {	
		_app.log(data);
			...
			_cp.render_view(_cp.views.tableView,data, 'x-projects');
			_cp.table = _cp.display_table('#tbl_projects');						
		
	});	
}

_cp.on.Add = function(){
	_app.nav_page('store.project_dtl')
	return false;
}


_cp.on.Edit = function(id){
	_app.nav_page('store.project_dtl',id)
	return false;
}
...
_cp.views.tableView = `
	<table class="table table-sm table-striped" id="tbl_projects">
	  ...
	  {% for item in projects %}
		...
		<tr>
		  ...
		  <td class="col">{{item.project_title}}</td>
