...
_cp.init = function(){
	_cp.views.page = './fe/app/25014/mod01/web/project/project_dtl.htm';
	_cp.api.item = '/be/app/25014/api/app25014_professional_hub/be/mod01/project/';
...
_cp.on.Cancel = function(){
	_app.nav_page('store.project_lst');
	return false;
}

_cp.on.Save = function(){
	_app.post_form($('#project_form'), _cp.api.item + _cp.params, function(resp){
		_app.nav_page('store.project_lst');
	});
	return false;
}
